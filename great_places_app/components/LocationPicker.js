...
const LocationPicker = props => {
    ...
    //This component receieves navigation props from the parent component so I can also get the params here.
    ...
    const getLocationHandler = async () => {
        ...
        try {
            setIsFetching(true);
            const location = await Location.getCurrentPositionAsync({ timeout: 5000 }); //Returns => accuracy, latitude, longitude, heading, speed
    ...
    return (
        <View style={styles.locationPicker}>
            <MapPreview style={styles.mapPreview} location={pickedLocation} onPress={pickOnMapHandler}>
                {isFetching ? (<ActivityIndicator size='large' color={Colors.primary} />) : (<Text>
